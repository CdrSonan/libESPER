...
void loopSamplerSpecharm(float* input, int length, float* output, int targetLength, float spacing, engineCfg config)
{
    ...
    else
    {
        ...
        for (int i = 0; i < effSpacing; i++)
        {
            ...
            {
                *(buffer + i * config.frameSize + j) *= (float)(i + 1) / (float)(effSpacing + 1);// powf((float)(i + 1) / (float)(effSpacing + 1), 4.);
                *(buffer + i * config.frameSize + j) += *(input + (length - effSpacing + i) * config.frameSize + j) * (1. - (float)(i + 1) / (float)(effSpacing + 1));// powf(1. - ((float)(i + 1) / (float)(effSpacing + 1)), 4.);
            }
